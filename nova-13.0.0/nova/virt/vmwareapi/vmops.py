...
class VMwareVMOps(object):
    ...
    def get_properties(self, value, _type, properties, mo_type):
        mo_ref = vim_util.get_moref(value, _type)
        ...
        #LOG.info("net_props: {}".format(net_props))
        query = vm_util.get_values_from_object_properties(
            self._session, props)

        return query

    def scheduler_by_memory(self, cluster_hosts, instance):
        
        vm_host =None
        select_host = None
        vm_host = vm_util.get_host_ref_for_vm(self._session,instance)
        if not vm_host:
            LOG.debug("The instance %s is not found in vCenter.",instance)
            return select_host

        LOG.info("vm_host_%s::::::::::",vm_host)
        for host_mor in cluster_hosts.ManagedObjectReference:
            if host_mor.value == vm_host.value:
                continue

            summary = self._session._call_method(vutil,
            ...
            data['ip'] = host_mor.value

            if not select_host:
                select_host = data
            elif select_host['host_memory_free'] < data['host_memory_free']:
                select_host = data

        LOG.info("search_memory_3:::::::::::::;;")
        LOG.info(select_host) 
        return select_host

    def live_migration(self, context, instance_ref, dest, post_method,
                        recover_method, block_migration=False, cluster_hosts=None):
        ...
        try:
            if CONF.vmware.vmware_migrate_scheduler == 'memoryScheduler':
                hostInfo = self.scheduler_by_memory(cluster_hosts,instance_ref)
                LOG.info("live_migration_memoryScheduler::::::::")
                LOG.info(hostInfo)
                if not hostInfo:
                    LOG.debug("The instance %s is not found in vCenter.",instance_ref)
                ...
                LOG.info("live_migration_no_memoryScheduler::::::::")
                vm_migrate_task = self._session._call_method(
                                    self._session.vim,
                                    "MigrateVM_Task", vm_ref,
                                    priority="defaultPriority")
