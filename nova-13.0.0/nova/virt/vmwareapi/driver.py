...
vmwareapi_opts = [
    cfg.StrOpt('vmware_migrate_scheduler',
               default='memoryScheduler',
               help='scheduler of vmware vms to migrate.'),
    cfg.StrOpt('host_ip',
...
class VMwareVCDriver(driver.ComputeDriver):
    ...
    def scheduler_by_memory(self):
        return vm_util.get_all_host_ref(self._session, self._cluster_ref)
    ...
    def live_migration(self, context, instance, dest,
        ...
        if CONF.vmware.vmware_migrate_scheduler == 'memoryScheduler':
            cluster_hosts = self.scheduler_by_memory()
            LOG.info("live_migration_1:::::::")
            LOG.info(cluster_hosts)
            if not cluster_hosts:
                LOG.debug("no valid host!")
                raise exception.NoValidHost()

        self._vmops.live_migration(context, instance, dest,
                                   post_method, recover_method,
                                   block_migration, cluster_hosts)
    ...
    def check_can_live_migrate_destination(self, context,
                                           instance,
                                           src_compute_info,
                                           dst_compute_info,
                                           block_migration,
                                           disk_over_commit):
        return migrate_data_obj.VMwareLiveMigrateData()

    def check_can_live_migrate_source(self, context,
                                           instance,
                                           dest_check_data,
                                           block_device_info=None):
