...
CONF = cfg.CONF
CONF.register_opts(vmware_utils_opts, 'vmwaredummy')
...
def get_vm_create_spec(client_factory, instance, data_store_name,
    ...
    if (CONF.vmwaredummy.console_delay_seconds and
        CONF.vmwaredummy.console_delay_seconds > 0):
        opt = client_factory.create('ns0:OptionValue')
        opt.key = 'keyboard.typematicMinDelay'
        opt.value = CONF.vmwaredummy.console_delay_seconds * 1000000
        extra_config.append(opt)
...
def create_serial_port_spec(client_factory):
    ...
    if not CONF.vmwaredummy.serial_port_service_uri:
        return

    backing = client_factory.create('ns0:VirtualSerialPortURIBackingInfo')
    backing.direction = "server"
    backing.serviceURI = CONF.vmwaredummy.serial_port_service_uri
    backing.proxyURI = CONF.vmwaredummy.serial_port_proxy_uri
...
def get_vnc_port(session):
    ...
    min_port = CONF.vmwaredummy.vnc_port
    port_total = CONF.vmwaredummy.vnc_port_total
