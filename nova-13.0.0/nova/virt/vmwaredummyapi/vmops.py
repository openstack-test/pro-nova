...
from nova.virt.vmwaredummyapi import ds_util
from nova.virt.vmwaredummyapi import error_util
from nova.virt.vmwaredummyapi import imagecache
from nova.virt.vmwaredummyapi import images
from nova.virt.vmwaredummyapi import vif as vmwarevif
from nova.virt.vmwaredummyapi import vim_util
...
CONF = nova.conf.CONF
CONF.register_opts(vmops_opts, 'vmwaredummy')
...
def retry_if_task_in_progress(f, *args, **kwargs):
    retries = max(CONF.vmwaredummy.api_retry_count, 1)
    delay = 1
...
class VMwareVMOps(object):
    ...
    def _get_base_folder(self):
        ...
        if CONF.vmwaredummy.cache_prefix:
            base_folder = '%s%s' % (CONF.vmwaredummy.cache_prefix,
    ...
    def spawn_reverse_manage_vm(self, context, instance, image_meta, injected_files,
        ...
        LOG.info("list_instances_0.01::::::")
        LOG.info(vc_uuid)
        LOG.info(vc_vm_uuid)

        if vc_uuid and vc_vm_uuid:
            #extra_opts = {'vc.uuid': "50 31 fb 61 8e d9 7f e4-6a f3 28 e3 72 97 74 4e"}
            extra_opts = {'vc.uuid': vc_uuid}
            client_factory = self._session.vim.client.factory
            reconfig_spec = vm_util.get_vm_extra_config_spec(client_factory, extra_opts)
            LOG.info("list_instances_0.1::::::")
            LOG.info(instance['uuid'])
            LOG.info(reconfig_spec)

            #vm_ref = vm_util._get_vm_ref_from_vm_uuid(self._session, '5031fb61-8ed9-7fe4-6af3-28e37297744e')
            vm_ref = vm_util._get_vm_ref_from_vm_uuid(self._session, vc_vm_uuid)
            #vm_ref =vm_util._get_vm_ref_from_name(self._session, instance['display_name']);
            LOG.info("list_instances_0.2::::::")
            LOG.info(vm_ref)
            extra_config = []
            ...
            #opt_iface = client_factory.create('ns0:OptionValue')
            #opt_iface.key = "nvp.iface-id.0"
            #opt_iface.value = network_info[0].get('id')
            #extra_config.append(opt_iface)
