...
from nova.virt.vmwaredummyapi import constants
from nova.virt.vmwaredummyapi import error_util
from nova.virt.vmwaredummyapi import host
from nova.virt.vmwaredummyapi import vim_util as nova_vim_util
from nova.virt.vmwaredummyapi import vm_util
from nova.virt.vmwaredummyapi import vmops
from nova.virt.vmwaredummyapi import volumeops
...
vmwaredummyapi_opts = [
    ...
    cfg.BoolOpt('use_raw_name',
                default=True,
                help='vm_name on vmware will be instance_name'),
...
CONF = nova.conf.CONF
CONF.register_opts(vmwaredummyapi_opts, 'vmwaredummy')
CONF.register_opts(spbm_opts, 'vmwaredummy')
...
class VMwareVCDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi, scheme="https"):
        ...
        if (CONF.vmwaredummy.host_ip is None or
            CONF.vmwaredummy.host_username is None or
            CONF.vmwaredummy.host_password is None):
            raise Exception(_("Must specify host_ip, host_username and "
                              "host_password to use vmwareapi.VMwareVCDriver"))

        self._datastore_regex = None
        if CONF.vmwaredummy.datastore_regex:
            try:
                self._datastore_regex = re.compile(CONF.vmwaredummy.datastore_regex)
            except re.error:
                raise exception.InvalidInput(reason=
                    _("Invalid Regular Expression %s")
                    % CONF.vmwaredummy.datastore_regex)
        ...
        if CONF.vmwaredummy.pbm_enabled:
            self._update_pbm_location()

        self._validate_configuration()
        self._cluster_name = CONF.vmwaredummy.cluster_name
    ...
    def _update_pbm_location(self):
        if CONF.vmwaredummy.pbm_wsdl_location:
            pbm_wsdl_loc = CONF.vmwaredummy.pbm_wsdl_location
        else:
    ...
    def _validate_configuration(self):
        if CONF.vmwaredummy.pbm_enabled:
            if not CONF.vmwaredummy.pbm_default_policy:
                raise error_util.PbmDefaultPolicyUnspecified()
            if not pbm.get_profile_id_by_name(
                            self._session,
                            CONF.vmwaredummy.pbm_default_policy):
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        vc_uuid =''
        vc_vm_uuid =''
        in_metadata = instance.get('metadata',{})
        if in_metadata :
            LOG.info("spawn_2::::::::::")
            LOG.info(in_metadata)
            if hasattr(in_metadata, "get"):
                vc_uuid = in_metadata.get("vc_uuid",{})
                vc_vm_uuid = in_metadata.get("vc_vm_uuid",{})

        if vc_uuid and vc_vm_uuid:
            self._vmops.spawn_reverse_manage_vm(context, instance, image_meta, injected_files,admin_password, network_info, block_device_info)
        else :
            self._vmops.spawn(context, instance, image_meta, injected_files,admin_password, network_info, block_device_info)
    ...
    def get_host_ip_addr(self):
        ...
        return CONF.vmwaredummy.host_ip
    ...
    def destroy(self, context, instance, network_info, block_device_info=None,
        ...
        if in_metadata :
            LOG.info(in_metadata)
            if hasattr(in_metadata, "get"):
                vc_uuid = in_metadata.get("soft_delete_vc_uuid",'')
                vc_vm_uuid = in_metadata.get("soft_delete_vc_vm_uuid",'')

        if vc_uuid and vc_vm_uuid:
            extra_opts = {'vc.uuid': vc_uuid}
            client_factory = self._session.vim.client.factory
            reconfig_spec = vm_util.get_vm_extra_config_spec(client_factory, extra_opts)
            vm_ref = vm_util._get_vm_ref_from_vm_uuid(self._session, vc_vm_uuid)
            ...
            reconfig_spec.extraConfig = extra_config
            LOG.info("list_instances_0.23::::::")

            reconfig_task = self._session._call_method(self._session.vim, "ReconfigVM_Task",
                                         vm_ref, spec=reconfig_spec)
            try:
                self._session._wait_for_task(reconfig_task)
            except Exception as e:
                LOG.info("exception in reconfigVM_Task by vcenter-api!")
        else:
...
class VMwareAPISession(api.VMwareAPISession):
    ...
    """
    def __init__(self, host_ip=CONF.vmwaredummy.host_ip,
                 host_port=CONF.vmwaredummy.host_port,
                 username=CONF.vmwaredummy.host_username,
                 password=CONF.vmwaredummy.host_password,
                 retry_count=CONF.vmwaredummy.api_retry_count,
                 scheme="https",
                 cacert=CONF.vmwaredummy.ca_file,
                 insecure=CONF.vmwaredummy.insecure):
        super(VMwareAPISession, self).__init__(
                ...
                task_poll_interval=CONF.vmwaredummy.task_poll_interval,
                scheme=scheme,
                create_session=True,
                wsdl_loc=CONF.vmwaredummy.wsdl_location,
                cacert=cacert,
                insecure=insecure)
