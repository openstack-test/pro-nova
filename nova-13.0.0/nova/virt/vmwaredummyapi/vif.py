...
from nova.virt.vmwaredummyapi import network_util
...
CONF.register_opts(vmwareapi_vif_opts, 'vmwaredummy')
...
def ensure_vlan_bridge(session, vif, cluster=None, create_vlan=True):
    ...
    vlan_interface = CONF.vmwaredummy.vlan_interface
...
def _get_opaque_network(session, cluster):
    ...
    except vexec.InvalidPropertyException:
        opaque = None
    return opaque
...
def _get_neutron_network(session, cluster, vif):
    ...
    if not opaque:
        network_name = (vif['network']['bridge'] or
                        CONF.vmwaredummy.integration_bridge)
        ridge = network_name
...
def get_vif_dict(session, cluster, vif_model, is_neutron, vif):
    mac = vif['address']
    name = vif['network']['bridge'] or CONF.vmwaredummy.integration_bridge
