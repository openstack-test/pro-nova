...
class RBDDriver(object):
    ...
    def cleanup_backup_volumes(self, backup_uuid):
        with RADOSClient(self, self.pool) as client:

            def belongs_to_backup(disk):
                return disk.startswith(backup_uuid)

            # pylint: disable=E1101
            volumes = rbd.RBD().list(client.ioctx)
            for volume in filter(belongs_to_backup, volumes):
                try:
                    rbd.RBD().remove(client.ioctx, volume)
                except (rbd.ImageNotFound, rbd.ImageHasSnapshots, rbd.ImageBusy):
                    LOG.warn(_LW('rbd remove %(volume)s in pool %(pool)s '
                                 'failed'),
                             {'volume': volume, 'pool': self.pool})

    def volume_copy(self, volume):

        with RADOSClient(self, self.pool) as client:
            try:
                rbd.RBD().remove(client.ioctx, str(volume+'-tmp'))
            except (rbd.ImageNotFound):
                pass

        args = [self.pool+'/'+volume,self.pool+'/'+volume+'-tmp']
        args += self.ceph_args()
        try:
            utils.execute('rbd', 'cp', *args, run_as_root=True)
        except (rbd.ImageNotFound):
            LOG.warn(_LW('rbd cp %(volume)s in pool %(pool)s '
                         'failed'),
                     {'volume': volume, 'pool': self.pool})

    def backup_resume_volumes(self, backup_volume, instance_volume):
        with RADOSClient(self, self.pool) as client:
            # pylint: disable=E1101
            try:
                LOG.info("ckup_resume_volumes_1::::")
                LOG.info(backup_volume)
                LOG.info(instance_volume)
                try:
                    rbd.RBD().remove(client.ioctx, str('last-'+instance_volume))
                except (rbd.ImageNotFound):
                    pass
                rbd.RBD().rename(client.ioctx, str(instance_volume), str('last-'+instance_volume))
                rbd.RBD().rename(client.ioctx, str(backup_volume), str(instance_volume))
            except (rbd.ImageNotFound, rbd.ImageHasSnapshots, rbd.ImageBusy):
                    LOG.warn(_LW('rbd rename %(volume)s in pool %(pool)s '
                                 'failed'),
                             {'volume': instance_volume, 'pool': self.pool}) 

    def export_volume(self, volume, path):
        args = [self.pool+'/'+volume,path]
        args += self.ceph_args()
        try:
            utils.execute('rbd', 'export', *args)
        except (rbd.ImageNotFound):
            LOG.warn(_LW('rbd export %(volume)s in pool %(pool)s '
                         'failed'),
                     {'volume': volume, 'pool': self.pool})
