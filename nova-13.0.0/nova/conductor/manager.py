...
class ComputeTaskManager(base.Base):
    ...
    def update_instance_image_info(image, instance, filter_properties):
        ...
        system_metadata = nova_utils.get_system_metadata_from_image(image, filter_properties.get('instance_type'))
        ...
        instance.system_metadata.setdefault('image_base_image_ref', instance['image_ref'])
        return instance

    def update_properties(self, image, instance, request_spec, filter_properties):
        instance = self.update_instance_image_info(image, instance, filter_properties)
        request_spec['instance_properties']['image_ref'] = image.get('id')
        request_spec['instance_properties']['system_metadata'] = instance.system_metadata
    ...
    def select_all_related_img(self, context, value):
        ...
        """
        target_images = []
        images = self.image_api.get_all(context)
        for img in images:
            if img.get('properties', {}).get('image_template') == value:
        ...
        if img_properties and img_properties.get('image_template'):
            ...
            for img_uuid in base_images:
                ...
                if img_info.get('properties', {}):
                    image_hv_type = img_info['properties'].get('hypervisor_type')
                    if image_hv_type and image_hv_type.lower() in host_hypervisor.lower():
                        return self.update_properties(img_info, instance, request_spec, filter_properties)

        return image, instance, request_spec

    @staticmethod
    def need_select_image(request_spec):
        """
        There'll be 'template' key/value in instance metadata
        if image should be re-selected.
        """
        inst_meta = request_spec.get('instance_properties', {}).get('metadata', None)
        ...
        for (instance, host) in six.moves.zip(instances, hosts):
            if reselect_flag:
                for hi in hosts_info:
                    if hi.get('service') and hi['service'].get('host') == host['host']:
                        host_hypervisor = hi.get('hypervisor_type')
                        LOG.debug('host_hypervisor: {0}'.format(host_hypervisor))
                        break

                image, instance, request_spec = self.select_image(
                    context, image, host_hypervisor, instance, request_spec, filter_properties)
