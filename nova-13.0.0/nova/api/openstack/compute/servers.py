...
class ServersController(wsgi.Controller):
    ...
    def _get_server_search_options(self, req):
        ...
        opt_list = ('reservation_id', 'name', 'status', 'image', 'flavor',
                    'ip', 'changes-since', 'all_tenants', 'metadata')
    ...
    def backup2_delete(self, req, id, body):
        ...
        instance = self._get_server(context, req, id)
        return self.compute_api.backup2_delete(context, instance, backup_id)
    ...
    def backup2_to_image(self, req, id, body):
        ...
        try:
            ...
            raise exc.HTTPBadRequest(explanation="backup status is not available")
    ...
    def _soft_instance_delete(self, context, req, instance_uuid, vc_uuid, vc_vm_uuid):
        instance = self._get_server(context, req, instance_uuid)
        instance.metadata['soft_delete_vc_uuid']=vc_uuid
        instance.metadata['soft_delete_vc_vm_uuid']=vc_vm_uuid
        LOG.info("_delete_1::::::::::::::::::::::::::")
    ...
    @wsgi.response(202)
    def soft_instance_delete(self, req, id, body):
        """Destroys a server is only nova."""
        
        if (not 'vc_uuid' in body):
            msg = _("No vc_uuid was specified")
            raise exc.HTTPBadRequest(explanation=msg)
        if (not 'vc_vm_uuid' in body):
            msg = _("No vc_vm_uuid was specified")
            raise exc.HTTPBadRequest(explanation=msg)
        
        vc_uuid = body.get("vc_uuid")
        vc_vm_uuid = body.get("vc_vm_uuid")
        try:
            self._soft_instance_delete(req.environ['nova.context'], req, id, vc_uuid, vc_vm_uuid)
