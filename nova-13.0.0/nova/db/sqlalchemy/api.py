...
def get_session(use_subordinate=False,context=None, **kwargs):
    ctxt_mgr = _context_manager_from_context(context) or main_context_manager
    return ctxt_mgr.get_legacy_facade().get_session(use_subordinate=use_subordinate, **kwargs)
...
def _check_backupid_exists(context, backup_id):
    if not model_query(context, models.Backup2, read_deleted="no").filter_by(
                       id=backup_id).first():
        raise exception.InstanceNotFound(instance_id=backup_id)
...
def backup2_create(context, values):
    ...
    session = get_session(context=context)
    with session.begin():
...
def backup2_get(context, backup_id):
    session = get_session(context=context)
    result = session.query(models.Backup2).\
                filter_by(id=backup_id).\
                first()

    if not result:
        raise exception.InstanceBackupNotFound(backup_id=backup_id)
...
def backup2_get_all(context):
    session = get_session(context=context)
    return session.query(models.Backup2).filter_by(deleted=0).all()

@require_context
def backup2_update(context, backup_id, values):
    session = get_session(context=context)
    with session.begin():
        snapshot_ref = backup2_get(context, backup_id)
