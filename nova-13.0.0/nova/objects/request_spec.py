...
class RequestSpec(base.NovaObject):
    ...
    def from_primitives(cls, context, request_spec, filter_properties):
        ...
        img_prop = image.get('properties', {})
        image_template = img_prop.get('image_template')
        if image_template:
            scheduler_hints['image_template'] = image_template
        metadata = instance.get('metadata')
        if metadata.get('template'):
            scheduler_hints['template'] = metadata.get('template')
