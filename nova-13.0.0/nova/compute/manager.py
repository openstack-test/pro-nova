...
class ComputeManager(manager.Manager):
    ...
    def backup2_instance_easy(self, context, instance, bak_info):
        context = context.elevated()
        LOG.info(_('instance backup2'), context=context,
                  instance=instance)
        #self._notify_about_instance_usage(context, instance, "backup2.start")
        ...
        #self._notify_about_instance_usage(context, instance, "backup2.end", backup_uuid=backup_id)
    ...
    def backup2_resume_instance(self, context, instance, bak_info):
        ...
        self._power_on(context, instance)
        current_power_state = self._get_power_state(context, instance)
        instance = self._instance_update(context, instance,
                power_state=current_power_state,
                vm_state=vm_states.ACTIVE,
                expected_task_state=task_states.BACKUP_RESUME,
                task_state=None)
    ...
    def backup2_delete(self, context, instance, bak_info):
        context = context.elevated()
        LOG.info(_('backup2_delete() Beginning'), context=context,
                  instance=instance)
        #self._notify_about_instance_usage(
