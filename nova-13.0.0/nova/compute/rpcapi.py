...
class ComputeAPI(object):
    ...
    def backup2_instance_easy(self, ctxt, instance, bak_info):
        cctxt = self.client.prepare(server=_compute_host(None, instance))
        cctxt.cast(ctxt, 'backup2_instance_easy', instance=instance,
                   bak_info=bak_info)

    def backup2_resume_instance(self, ctxt, instance, bak_info):
        cctxt = self.client.prepare(server=_compute_host(None, instance))
        cctxt.cast(ctxt, 'backup2_resume_instance', instance=instance,
                   bak_info=bak_info)      
    ...
    def backup2_delete(self, ctxt, instance, bak_info):
        ...
        cctxt = self.client.prepare(server=_compute_host(None, instance),
                version=version)
        instance_p = jsonutils.to_primitive(instance)
        cctxt.cast(ctxt, 'backup2_delete', instance=instance,
                   bak_info=bak_info)
