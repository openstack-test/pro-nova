...
class API(base.Base):
    ...
    def _create_instance(self, context, instance_type,
        ...
        #add isvm=1 fow vm and islogic=1 for phy-vm by metadata.
        LOG.info("create_instances_1::::::::::::")
        if hasattr(metadata,'get'):
            isvm=metadata.get('isvm','')
            islogic=metadata.get('islogic','')
            if not isvm and not islogic:
                LOG.info("need to add isvm or islogic!")
                if hasattr(boot_meta,'get'):
                    LOG.info("hypervisor_type_1:::::::")
                    image_properties = boot_meta.get('properties','')
                    if image_properties:
                        image_hypervisor_type = image_properties.get('hypervisor_type','')
                        if image_hypervisor_type and image_hypervisor_type in [hv_type.HYPERV,hv_type.QEMU,hv_type.VMWARE_LEGACY,hv_type.VMWARE]:
                            LOG.info('isvm')
                            metadata['isvm']='1'
                        elif image_hypervisor_type and image_hypervisor_type == hv_type.BAREMETAL:
                            LOG.info('islogic')
                            metadata['islogic']='1'
                    else:
                        msg = (_('Image %s hypervistor_type expected to be one type in [HYPERV,QEMU,VMWARE_LEGACY,VMWARE,BAREMETAL], but it was none.') % image_id)
                        LOG.error(msg)
            else:
                msg = (_("Image %s properties is None,so don't confirm hypervisor_type.") % image_id)
                LOG.warning(msg)
    ...
    def get_all(self, context, search_opts=None, limit=None, marker=None,
        ...
        # add for sort-key
        if search_opts.has_key('sort_key'):
            sort_key = search_opts.get('sort_key')
        if search_opts.has_key('sort_dir'):
            sort_dir = search_opts.get('sort_dir')
    ...
    def _initialize_instance_snapshot_metadata(self, instance, name,
        ...
        if properties.get('image_template'):
            properties.pop('image_template', None)
    ...
    def backup2_resume_instance(self, context, instance, backup_id):
        ...
        bak_info = self.backup_info_convert(res)
        if res['backup_status'] != 'active':
            raise exception.Invalid
         
        instance.task_state = task_states.BACKUP_RESUME
        instance.save(expected_task_state=[None])
        self._record_action_start(context, instance, instance_actions.RESUME)
    ...
    def backup_info_convert(self, res):
        item = {}
        item['id'] = res.get('id')
        item['name'] = res.get('display_name')
        #item['description'] = res.get('display_description', None)
    ...
    @wrap_check_policy
    def backup2_delete(self, context, instance, backup_id):
        res = self.db.backup2_get(context, backup_id)
    ...
    def backup2_to_image(self, context, req, backup_id, name,
        ...
        bak_info = self.backup_info_convert(info)
        ...
        if instance_id:
            ...
            image_meta = self._create_image(context, instance, name,
                                            'snapshot',
                                            extra_properties=extra_properties)
            self.compute_rpcapi.backup2_to_image(context, instance, bak_info,
                                                 image_meta['id'])
            return image_meta    
